...
<style>
    ...
    .user-avatar { width:28px; height:28px; border-radius:6px; object-fit:cover; }
    .user-fallback { font-size:22px; width:28px; height:28px; display:flex; align-items:center; justify-content:center; }
    .user-name { margin:0; font-size:0.95rem; font-weight:600; }
</style>
@endsection

@section('content')
<div class="main-content">
    <h2 class="mb-3">{{ ucFirst($status) }} Freelancers</h2>
    ...
    <div class="table-container">
        <table id="freelancers-table" class="table table-striped w-100">
            <thead>
                <tr>
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="badge bg-dark">
                                {{ optional($user->freelancerTaskOffers)->count() ?? 0 }}
                            </span>
                        </td>
                                ...
                                <div class="dropdown-menu">
                                    @foreach ($allStatus as $st)
                                        <a class="dropdown-item {{ $user->status == $st['name'] ? 'd-none' : '' }}"
                                           {{-- href="{{ route('adminUpdateFreelancerstatus', ['id' => $user->id, 'status' => $st['name']]) }}" --}}
                                        >
                                            {{ ucFirst($st['name']) }}
                                        </a>
                                    @endforeach
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                ...
                                <a href="{{ route('singleFreelancer', $user->id) }}"
                ...
                @empty
                    <tr><td colspan="7" class="text-center text-muted">No freelancers found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
