        ...
        <div class="row d-none">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title">{{ucFirst($status)}} Tasks</h4>
                </div>
            </div>
        </div>
        <!-- start page title -->
                                    ...
                                    @if (count($tasks) > 0)
                                        @foreach ($tasks as $item)
                                            <tr>
                                                ...
                                                <td>
                                                    {{ $item->freelancer ? $item->freelancer->name : 'N/A' }}
                                                </td>

                                                <td>{{ $item->task_title }}</td>
                                                <td>{{ $item->createdBy->current_address ? $item->createdBy->current_address : 'N/A' }}</td>
                                                <td>{{ $item->task_date }}</td>
                                                <td>{{ $item->task_budget }}</td>
                                                        ...
                                                        <div class="dropdown-menu">
                                                            @foreach ($allStatus as $status)
                                                                <a class="dropdown-item {{ $item->status == $status['name'] ? 'd-none' : '' }}"
                                                                    {{-- href="{{ route('adminUpdateTaskstatus', ['id' => $item->id, 'status' => $status['name']]) }}" --}}
                                                                >
                                                                    {{ ucFirst($status['name']) }}
                                                                </a>
                                                            @endforeach
